...
public class BOJ_15652 {
  ...
  public static void main(String[] args) throws IOException {
    ...
    dfs(result, range, m, 0, new ArrayDeque<>());
    for (int[] arr : result) {
  ...
  public static void dfs(SortedSet<int[]> result, List<Integer> range, int size, int start,
    ...
    for (int i = start; i < range.size(); i++) {
      ...
      dfs(result, range, size - 1, i, combination);
